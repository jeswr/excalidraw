...
// suite's tested, E2E-verified login layer. Copied verbatim; do not diverge from the
// upstream source (re-sync from the skill rather than editing in place).
/**
...
export function resolveIssuers(webId: string, dataset: DatasetCore): string[] {
  ...
  if (issuers.length === 0) {
    throw new NoSolidIssuerError(webId);
  }
  return issuers;
}
