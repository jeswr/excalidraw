// VENDORED from jeswr/solid-ai-coding (skills/solid-reactive-authentication) — the
// suite's tested, E2E-verified WebID-first DPoP TokenProvider with the static
// client_id hook the published @solid/reactive-authentication 0.1.2 lacks. Copied
// verbatim; re-sync from the skill rather than editing in place.
/**
...
export class WebIdDPoPTokenProvider implements TokenProvider {
  ...
  async #authenticate(
    ...
    const authorizationCodeResponse = await this.#getCode(
      authorizationUrl,
      signal,
    );
    try {
      ...
      if (
        ...
        if (usePkce) {
          retryUrl.searchParams.set("code_challenge", codeChallenge);
        }
  ...
  #nonceVerification(
    issuer: string,
    nonce: string,
  ): string | typeof oauth.expectNoNonce {
    if (
...
function clientSecretBasicFor(
  issuer: string,
): (secret: string) => oauth.ClientAuth {
